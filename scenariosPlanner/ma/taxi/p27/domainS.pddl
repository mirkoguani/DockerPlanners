...
(:functions
    (goal-of-passenger-reached ?p - passenger)
    (time-travelled ?t - taxi)
)
...
(:durative-action drive
	...
	:effect (and
		...
		(at end (xfreex ?t))
		(at end (increase (time-travelled ?t) 10))
	)
)
...
(:durative-action enter
	...
	:effect (and
		...
		(at start (not (xfreex ?t)))
		(at end (xfreex ?t))
		(at end (increase (time-travelled ?t) 1))
	)
)
...
(:durative-action exit
	...
	:effect (and
		...
		(at start (not (xfreex ?t)))
		(at end (xfreex ?t))
		(at end (increase (goal-of-passenger-reached ?p) 1))
		(at end (increase (time-travelled ?t) 1))
	)
)
