...
//  LPScheduler tryToSchedule(header, now, theState.entriesForAction, (prevState ? &prevState->secondMin : 0), (prevState ? &prevState->secondMax : 0));
    ...
    unique_ptr<RPGHeuristic::EvaluationInfo> h;//h(0);
    ...
    if (considerCache) {
        if (FFcache_upToDate) {
            ...
            h = std::move(unique_ptr<RPGHeuristic::EvaluationInfo>(new RPGHeuristic::EvaluationInfo(FFcache_h)));
            ...
            h = std::move(unique_ptr<RPGHeuristic::EvaluationInfo>(RPGBuilder::getHeuristic()->getRelaxedPlan(theState.getEditableInnerState(), &(theState.startEventQueue), minTimestamps, theState.timeStamp, costLimit,
                                                                                                   extrapolatedMin, extrapolatedMax, timeAtWhichValueIsDefined,                                  // for colin-jair heuristic
    ...
    if (h->goalState) {
        ...
        if (FF::relaxMIP) {
            ...
            mipModel = std::move(unique_ptr<LPScheduler>(new LPScheduler(theState.getInnerState(), theState.getEditableInnerState().preferenceStatus, header, now, stepID, theState.startEventQueue, incrementalData, theState.entriesForAction, (prevState ? &prevState->getInnerState().secondMin : 0), (prevState ? &prevState->getInnerState().secondMax : 0), &(theState.tilComesBefore), scheduleToMetric)));
    ...
    while (!searchQueue.empty()) {
        ...
        for (; helpfulActsItr != helpfulActsEnd; ++helpfulActsItr) {
            ...
            if (!tsSound) {
                ...
                unique_ptr<SearchQueueItem> TILparentAutoDelete;
                ...
                unique_ptr<StateHash::InsertIterator> insResult;
                bool visitTheState = false;
