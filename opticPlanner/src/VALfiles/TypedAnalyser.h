...
private:
	bool initialFactsAndFluents;
    bool timedFactsAndFluents;
	double when;
	bool finally;
	bool pos;
	bool adding;
...
public:
	...
	virtual void visit_timed_initial_literal(timed_initial_literal * p)
	{
        assert(initialFactsAndFluents);
        initialFactsAndFluents = false;
        timedFactsAndFluents = true;
		when = p->time_stamp;
		p->effs->visit(this);
		when = 0;
	...
	virtual void visit_assignment(assignment * p)
	{
		switch(p->getOp())
		{
			case E_ASSIGN:
                ...
                {
                    EFT(p->getFTerm()->getFunction())->addAssign(0,p);
                }
                else
				{
